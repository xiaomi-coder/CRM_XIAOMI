...
const Login: React.FC<LoginProps> = ({ t, onLogin, onTestLogin, error: externalError }) => {
  ...
  return (
    <div className="min-h-screen bg-[#0A0F0D] flex items-center justify-center p-4 relative overflow-hidden font-sans">
      <div className="login-blob absolute top-[-20%] left-[-10%] w-[60%] h-[60%] bg-emerald-600/10 rounded-full blur-[120px]"></div>
      <div className="login-blob-2 absolute bottom-[-20%] right-[-10%] w-[60%] h-[60%] bg-teal-500/10 rounded-full blur-[120px]"></div>

      <div className="w-full max-w-md z-10">
        <div className="login-card bg-white/95 backdrop-blur-2xl rounded-[3rem] shadow-2xl overflow-hidden border border-white/20">
          <div className="bg-gradient-to-br from-emerald-700 via-emerald-600 to-teal-700 p-10 text-center text-white relative">
            <div className="login-logo inline-flex p-4 bg-white/10 rounded-[1.5rem] mb-4 backdrop-blur-xl border border-white/20 shadow-inner group">
              <BrainCircuit size={48} />
            </div>
            <h1 className="text-3xl font-black tracking-tighter italic mb-1">EduControl</h1>
            <p className="text-emerald-100/80 text-[10px] font-bold uppercase tracking-[0.3em]">Online CRM System</p>
          </div>

          <div className="flex p-2 bg-slate-100 mx-8 mt-6 rounded-2xl">
            <button
              onClick={() => setMode('ADMIN')}
              className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${mode === 'ADMIN' ? 'bg-white text-emerald-600 shadow-md' : 'text-slate-400'}`}
            >
              {t.staff || 'Staff'}
            </button>
            <button
          ...
          <form onSubmit={handleSubmit} className="login-stagger p-8 space-y-6">
            {displayError && (
                ...
                <>
                  <div className="space-y-1.5">
                    ...
                    <div className="relative group">
                      <UserIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-emerald-500 transition-colors" size={18} />
                      <input
                        type="text"
                        className="w-full pl-12 pr-6 py-4 bg-gray-50 border border-gray-100 rounded-[1.2rem] outline-none font-bold text-gray-700 focus:ring-2 focus:ring-emerald-500 transition-all"
                        placeholder={t.login}
                        value={username}
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-center">
                    ...
                    <input
                      type="text"
                      maxLength={6}
                      className="w-full text-center py-5 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] outline-none font-black text-4xl tracking-[0.5em] text-amber-600 focus:border-amber-500 transition-all"
                      placeholder="000000"
                      value={testPin}
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full text-white font-black py-4 bg-emerald-600 rounded-[1.2rem] shadow-xl shadow-emerald-600/20 hover:bg-emerald-700 hover:-translate-y-0.5 active:scale-[0.97] transition-all flex items-center justify-center gap-3 text-sm uppercase tracking-widest disabled:opacity-50"
            >
          ...
          <div className="p-8 border-t border-gray-100 flex flex-col items-center gap-4 bg-gray-50/50">
            <div className="flex items-center gap-6">
              <a href="tel:+998906127171" className="flex items-center gap-2 text-emerald-600 hover:scale-105 transition-transform">
                <Phone size={14} />
