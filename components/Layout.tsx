...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, user, onLogout }) => {
  ...
  return (
    <div className="flex min-h-screen bg-[#f8f9fc]">
      <aside className="w-72 bg-[#0a0d14] text-white flex flex-col fixed h-full z-20 shadow-pop">
        <div className="p-8 flex items-center space-x-4 border-b border-white/5">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tighter italic uppercase leading-none">EDUCONTROL</h1>
            <p className="text-[7px] font-bold text-slate-500 uppercase tracking-[0.4em] mt-1.5">Professional CRM</p>
          </div>
        </div>
        ...
        <nav className="flex-1 px-4 mt-8 space-y-8 overflow-y-auto custom-scrollbar">
          {sections.map((section, idx) => (
            <div key={idx} className="space-y-2">
              <h3 className="px-4 text-[8px] font-bold text-slate-600 uppercase tracking-[0.2em] mb-4">{section.title}</h3>
              {section.items.map(item => (
                <button
                  key={item.id}
                  onClick={() => setActiveTab(item.id)}
                  className={`w-full flex items-center space-x-3 px-5 py-4 rounded-2xl transition-all duration-300 group ${activeTab === item.id ? 'bg-[#ffc107] text-black shadow-card scale-[1.02]' : 'text-slate-400 hover:text-white hover:bg-white/5'
                    }`}
                >
                  <item.icon size={18} className={activeTab === item.id ? 'stroke-[2.5px]' : 'opacity-40'} />
                  <span className={`text-[12px] ${activeTab === item.id ? 'font-bold' : 'font-bold uppercase tracking-tight'}`}>{item.label}</span>
                </button>
              ))}
            </div>
          ))}
        ...
        <div className="p-6 mt-auto border-t border-white/5 bg-white/[0.02]">
          {hasPermission('settings') && (
            ...
            >
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest">{t.system_settings}</span>
            </button>
          )}

          <div className="flex items-center gap-4 px-2 mb-4">
            <div className="w-10 h-10 rounded-xl bg-indigo-600/20 flex items-center justify-center font-bold text-indigo-400 border border-indigo-500/20">
              {user.name.charAt(0)}
            </div>
            <div className="flex flex-col min-w-0">
              <span className="text-[11px] font-bold text-white truncate">{user.name}</span>
              <span className="text-[7px] font-bold text-indigo-400 uppercase tracking-widest truncate">{roleLabel}</span>
            </div>
          </div>

          <button onClick={onLogout} className="w-full flex items-center justify-center gap-2 py-3 text-[9px] font-bold uppercase text-red-500/60 hover:text-red-500 hover:bg-red-500/5 rounded-xl transition-all">
      ...
      <main className="flex-1 ml-72 p-10">
        <header className="mb-10 flex items-center justify-between bg-white p-8 rounded-card shadow-sm border border-slate-100">
          <div className="flex flex-col">
            <p className="text-[9px] font-bold text-indigo-500 uppercase tracking-[0.5em] mb-1.5 ml-1">{t.online_system}</p>
            <h2 className="text-4xl font-bold text-slate-900 tracking-tighter uppercase italic leading-none">
              {activeTab === 'settings' ? t.system_settings : (sections.flatMap(s => s.items).find(i => i.id === activeTab)?.label || t.dashboard)}
            </h2>
          </div>
          <div className="bg-[#0a0d14] px-10 py-5 rounded-card shadow-pop flex items-center gap-10 border border-white/5">
            <div className="flex items-center gap-8 border-r border-white/10 pr-10">
              <div className="flex items-center gap-4">
                <Clock className="text-indigo-500" size={24} />
                <span className="text-3xl font-bold text-white font-mono tracking-widest">{formatClock(currentTime)}</span>
              </div>
              <div className="flex bg-white/5 p-1 rounded-xl">
                {(['uz', 'ru', 'en'] as Language[]).map(l => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase transition-all ${lang === l ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
                  >
                    {l}
                  </button>
                ))}
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col text-white">
                <span className="text-[13px] font-bold leading-none">{dateStr}</span>
                <span className="text-[9px] font-bold text-indigo-400 uppercase tracking-widest mt-1.5 italic transition-all">{weekday}</span>
              </div>
            </div>
          </div>
        </header>
