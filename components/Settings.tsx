    ...
    <div className="max-w-6xl mx-auto space-y-12 pb-32">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {userRole === UserRole.DIRECTOR && (
          <div className="bg-white p-10 rounded-card border border-slate-100 shadow-pop">
            <div className="flex items-center gap-4 mb-8">
              <div className="bg-indigo-600 p-4 rounded-3xl shadow-lg"><Building2 className="text-white" size={24} /></div>
              <h3 className="text-2xl font-bold text-slate-800 uppercase italic">{t.settings}</h3>
            </div>
            <form onSubmit={async (e) => {
              ...
              <div className="p-6 bg-slate-900 rounded-card text-white space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Bot size={18} className="text-indigo-400" />
                    <span className="text-[10px] font-bold uppercase tracking-widest">Telegram Bot</span>
                  </div>
                ...
                <div className="p-6 bg-slate-900 rounded-card text-white space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-widest">AI Integration (Gemini)</span>
                  </div>
                ...
                <div className="p-6 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-card text-white space-y-4">
                  <div className="flex items-center gap-2">
                    <Bot size={18} className="text-blue-200" />
                    <span className="text-[10px] font-bold uppercase tracking-widest">Kunlik Hisobot (Telegram)</span>
                  </div>
              ...
              <button type="submit" className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold uppercase shadow-card">{t.save}</button>
            </form>
          </div>
        )}

        <div className={`bg-white p-10 rounded-card border border-slate-100 shadow-pop ${userRole !== UserRole.DIRECTOR ? 'lg:col-span-2' : ''}`}>
          <div className="flex justify-between items-center mb-8">
            <div className="flex items-center gap-4">
              <div className="bg-amber-500 p-4 rounded-3xl shadow-lg"><BookOpen className="text-white" size={24} /></div>
              <h3 className="text-2xl font-bold text-slate-800 uppercase italic">{t.tests}</h3>
            </div>
            <button
              ...
              className="px-6 py-3 bg-amber-50 text-amber-600 rounded-2xl hover:bg-amber-500 hover:text-white transition-all font-bold uppercase text-[10px] tracking-widest"
            >
              + {t.add_test}
            </button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {templates.map(test => (
              <div key={test.id} className="p-6 bg-slate-50 rounded-card border border-indigo-100 flex flex-col justify-between hover:shadow-lg transition-all relative group">
                ...
                <div>
                  <p className="font-bold text-slate-800 text-sm uppercase mb-1">{test.title}</p>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{test.subject} • {test.questions.length}</p>
                </div>
                <div className="mt-6">
                  <button
                    type="button"
                    onClick={() => exportToWord(test)}
                    className="w-full bg-white border border-slate-200 p-3 rounded-2xl hover:bg-blue-50 hover:text-blue-600 transition-all flex items-center justify-center gap-2 text-[9px] font-bold uppercase"
                  >
      ...
      {showAddTestModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[200] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-3xl rounded-card shadow-pop overflow-hidden animate-in zoom-in duration-300 max-h-[90vh] flex flex-col">
            ...
            <div className="p-10 overflow-y-auto custom-scrollbar space-y-8">
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center border-b pb-4">
                  <h4 className="font-bold text-slate-800 uppercase text-xs">{t.question} ({newTemplate.questions?.length || 0})</h4>
                  <button onClick={() => setNewTemplate(p => ({ ...p, questions: [...(p.questions || []), { id: Math.random().toString(), text: '', options: ['', '', '', ''], correctAnswer: 0 }] }))} className="bg-indigo-600 text-white px-5 py-2 rounded-xl text-[10px] font-bold uppercase text-nowrap">+ {t.question}</button>
                </div>
                {newTemplate.questions?.map((q, qIdx) => (
                  <div key={q.id} className="p-6 bg-slate-50 rounded-card border border-slate-200 space-y-4">
                    <div className="flex justify-between">
                      <span className="font-bold text-[10px] uppercase text-indigo-500">{t.question} {qIdx + 1}</span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {q.options.map((opt, i) => (
                        <div key={i} className={`flex items-center gap-3 p-3 rounded-xl border ${q.correctAnswer === i ? 'bg-emerald-50 border-emerald-500' : 'bg-white'}`}>
                          ...
                          <span className="font-bold text-[12px] text-slate-400">{optionLetters[i]}</span>
                          <input
            ...
            <div className="p-8 bg-slate-50 border-t flex gap-4">
              <button type="button" onClick={() => setShowAddTestModal(false)} className="flex-1 font-bold uppercase text-xs text-slate-400">{t.cancel}</button>
              <button type="button" onClick={handleSaveTemplate} className="flex-1 bg-amber-500 text-white font-bold py-4 rounded-2xl uppercase text-xs shadow-card">
                {t.save}
              </button>
            </div>
          </div>
